...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T)
    SystemIDlookuptable_M->Timing.clockTick0;
  SystemIDlookuptable_step();
...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(SystemIDlookuptable_M, 0);
  ...
  SystemIDlookuptable_initialize();
  globalInterruptDisable();
  globalInterruptEnable();

  /* External Mode initialization */
  errorCode = extmodeInit(SystemIDlookuptable_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(SystemIDlookuptable_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(SystemIDlookuptable_M);
  enableTimer0Interrupt();
  config_ePWM_TBSync();
  globalInterruptEnable();
  while (runModel) {
