...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(HW2_hardware_file_M, 0);
  ...
  HW2_hardware_file_initialize();
  globalInterruptDisable();
  globalInterruptEnable();

  /* External Mode initialization */
  errorCode = extmodeInit(HW2_hardware_file_M->extModeInfo, &rtmGetTFinal
    (HW2_hardware_file_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(HW2_hardware_file_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(HW2_hardware_file_M);
  enableTimer0Interrupt();
  config_ePWM_TBSync();
  globalInterruptEnable();
  while (runModel) {
